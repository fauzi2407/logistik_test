...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-sm flex items-center justify-between">
        <div>
            <h2 class="text-xl font-extrabold text-slate-900 tracking-tight">Form Input Customer Baru</h2>
            <p class="text-xs text-slate-500 mt-0.5">Lengkapi profil data customer beserta alamat hierarki wilayah Indonesia.</p>
        </div>
    ...
    <form action="{{ route('customers.store') }}" method="POST" class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="name" required placeholder="Contoh: Bpk. Ahmad Fauzi" class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="company_name" placeholder="Contoh: PT Nusantara Jaya" class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Nomor Telepon / WhatsApp *</label>
                <input type="text" name="phone" required placeholder="Contoh: 081299887766" class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="email" name="email" placeholder="customer@domain.com" class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800">
            </div>

            <div class="flex items-center pt-5">
                <label class="inline-flex items-center space-x-2 text-xs font-bold text-slate-700 cursor-pointer">
                    <input type="checkbox" name="create_account" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4">
                    <span>Buatkan Akun Portal Customer (Login)</span>
                </label>
            </div>
        </div>
        ...
        <div class="space-y-2 pt-2 border-t border-slate-100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 text-xs bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div>
                    ...
                    <input type="text" name="postal_code" id="cust-postal-input" placeholder="12910" class="w-full px-2.5 py-2 rounded-xl border border-slate-300 font-bold text-indigo-600">
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="address" rows="3" required placeholder="Jl. Jend. Sudirman Kav. 76-78, Gedung Plaza Indofood Lt. 23..." class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800"></textarea>
        </div>
        ...
        <div class="pt-4 flex items-center justify-end space-x-3 border-t border-slate-100">
            ...
            <button type="submit" class="px-6 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md shadow-indigo-600/30">
                <i class="fa-solid fa-save mr-1"></i> Simpan Customer
            </button>
        </div>
    </form>
</div>
